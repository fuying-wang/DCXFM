    ...
    def forward(self, ids, attn_mask, token_type):
        ...
        if self.last_n_layers > 1:
            ...
            if self.agg_tokens:
                ...
                report = [[self.idxtoword[w.item()] for w in sent]
                          for sent in ids]
            ...
            if self.aggregate_method == "sum":
                ...
                if self.take_sent_as_units:
                    sent_embeddings, sent_units_report = self.aggregate_tokens_sent_as_units(
                        embeddings, report)
        ...
        if self.take_sent_as_units:
            ...
            sent_embeddings = sent_embeddings.view(
                batch_dim * num_words, feat_dim)
            if self.emb_local is not None:
