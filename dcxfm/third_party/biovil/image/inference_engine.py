...
class ImageInferenceEngine:
    ...
    def get_projected_patch_embeddings(self, image: torch.Tensor) -> Tuple[torch.Tensor, TypeShape2D]:
        ...
        input_image = image
        img_shape = [input_image.shape[2], input_image.shape[3]]
        ...
        return projected_img_emb, img_shape

    @torch.no_grad()
    # def get_projected_global_embedding(self, image_path: Path) -> torch.Tensor:
    def get_projected_global_embedding(self, image: torch.Tensor) -> torch.Tensor:
        ...
        projected_img_emb = self.model.forward(image).projected_global_embedding
