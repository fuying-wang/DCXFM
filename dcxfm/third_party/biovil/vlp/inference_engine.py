...
class ImageTextInferenceEngine:
    ...
    def get_similarity_score_from_raw_data(self, images: torch.Tensor, 
        ...
        image_embedding = self.image_inference_engine.get_projected_global_embedding(images)
        # text_embedding = self.text_inference_engine.get_embeddings_from_prompt(query_text, normalize=False)
        text_embedding = self.text_inference_engine.get_embeddings_from_tokens(input_ids, attention_mask, normalize=True)
    ...
    def get_similarity_map_from_raw_data(
        ...
        self, image: torch.Tensor, query_text: str, interpolation: str = "nearest"
    ) -> np.ndarray:
        ...
        image_embedding, (width, height) = self.image_inference_engine.get_projected_patch_embeddings(image)
