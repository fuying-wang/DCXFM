...
class CLP_clinical(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.bert_model = self._get_bert_basemodel(
            bert_model_name=bert_model_name, freeze_layers=freeze_layers)
    ...
    def _get_bert_basemodel(self, bert_model_name, freeze_layers=None):  # 12
        try:
            ...
            config = BertConfig.from_pretrained(
                bert_model_name, output_hidden_states=True)  # bert-base-uncased
            model = AutoModel.from_pretrained(
                bert_model_name, config=config)  # , return_dict=True)
...
class ModelDense(nn.Module):
    ...
    def _get_dense_basemodel(self, dense_base_model):
        try:
            ...
            raise (
                "Invalid model name. Check the config file and pass one of: densenet121 or densenet161")
