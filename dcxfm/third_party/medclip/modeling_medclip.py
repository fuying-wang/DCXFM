...
from torchvision.models.resnet import BasicBlock, Bottleneck, WeightsEnum, _ovewrite_named_param, ResNet50_Weights
import torch
...
class MedCLIPVisionModel(nn.Module):
    ...
    def __init__(self, checkpoint=None, medclip_checkpoint=None):
        ...
        self.model = resnet50(
            weights=ResNet50_Weights.IMAGENET1K_V1, progress=True)
    ...
    def forward(self, pixel_values, dense=True, **kwargs):
        '''args:
        ...
        if dense:
            patch_feats = self.model(pixel_values)
            img_feats = F.adaptive_avg_pool2d(
                patch_feats, (1, 1)).reshape(patch_feats.shape[0], -1)
            from einops import rearrange
            patch_feats = rearrange(patch_feats, "b c h w -> b (h w) c")
            patch_embeds = self.model.fc(patch_feats)
            img_embeds = self.model.fc(img_feats)
            return img_embeds, patch_embeds
        else:
            patch_feats = self.model(pixel_values)
            img_feats = F.adaptive_avg_pool2d(
                patch_feats, (1, 1)).reshape(patch_feats.shape[0], -1)
...
class MedCLIPVisionModelViT(nn.Module):
    ...
    def forward(self, pixel_values, project=True, dense=True):
        '''args:
        pixel_values: tensor with shape [bs, 3, img_size, img_size]
        '''
        if pixel_values.shape[1] == 1:
        ...
        # The shared projection head seems to be wierd.
        if dense:
            patch_embeds = output['last_hidden_state']
            # TODO
            patch_embeds = self.projection_head(patch_embeds)
            return img_embeds, patch_embeds
        else:
            return img_embeds
...
class MedCLIPModel(nn.Module):
    ...
    def encode_image(self, pixel_values=None, dense=False):
        # image encoder
        vision_output = self.vision_model(
            pixel_values=pixel_values, dense=dense)
        if dense:
            img_embeds, patch_embeds = vision_output
            img_embeds = img_embeds / img_embeds.norm(dim=-1, keepdim=True)
            patch_embeds = patch_embeds / \
                patch_embeds.norm(dim=-1, keepdim=True)
            return img_embeds, patch_embeds
        else:
            img_embeds = vision_output / \
                vision_output.norm(dim=-1, keepdim=True)
            return img_embeds
