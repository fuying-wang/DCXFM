...
'''
These segmentation metrics are borrowed from: 
...
class BinaryMetrics():
    ...
    """
    def __init__(self, eps=1e-6, activation='0-1'):
        self.eps = eps
        self.activation = activation

    def _calculate_overlap_metrics(self, gt, pred):
        output = pred.reshape(-1)
        target = gt.reshape(-1).astype(np.float32)
    ...
    def __call__(self, y_pred, y_true):
        ...
        if self.activation in [None, 'none']:
            ...
            activated_pred = (sigmoid_pred > 0.5).astype(np.float32)
        else:
            raise NotImplementedError("Not a supported activation!")

        assert y_true.shape == y_pred.shape, "y_true and y_pred must have the same shape."
        iou, dice = self._calculate_overlap_metrics(y_true, activated_pred)
        # pixel_acc, dice, precision, specificity, recall = self._calculate_overlap_metrics(y_true,
        #                                                                                   activated_pred)
        # return [pixel_acc, dice, precision, specificity, recall]
        return iou, dice